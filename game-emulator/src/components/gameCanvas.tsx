...
export default function GameCanvas({buttonList, currentButton}: {buttonList: string[], currentButton?: string}) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  // Character State, so we can refer later
  const player = useRef({ x: 50, y: 150, color: 'blue', action: 'idle', ammo: 10 }); // to track player position and state, without causing re-renders
  ...
  useEffect(() => {
    ...
    const update = () => {
      if (currentButton === 'RIGHT' && player.current.x < 300) player.current.x += 4;
      if (currentButton === 'LEFT' && player.current.x > 10) player.current.x -= 4;
      if (currentButton === 'UP' && player.current.y > 10) player.current.y -= 4;
      if (currentButton === 'DOWN' && player.current.y < 190) player.current.y += 4; // good gaurds to prevent moving out of canvas bounds
      ...
      if (ctx) {
        ...
        if (player.current.action === 'throw') {
          ctx.fillStyle = 'black';
          ctx.beginPath();
          ctx.arc(player.current.x + 20, player.current.y + 10, 5, 0, Math.PI * 2); // Simple projectile
          ctx.fill();
        }
      }
